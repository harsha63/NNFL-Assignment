...
p = randperm(length(table1)); %dividing data to 70% training data and 30% test data 
...
u1 = mean(t1(:,1:end-1))'; 
u2 = mean(t2(:,1:end-1))';
u3 = mean(t3(:,1:end-1))'; %calculating covariance matrix and determinant of covariance matrix
...
x = test_data(i,1:end-1)'; %calculating p(x/yk)
...
p_x_y = [p_x_y1*py1 p_x_y2*py2 p_x_y3*py3]; 
%calculating p(yk/x) 
[~, yp(i)] = max(p_x_y);
